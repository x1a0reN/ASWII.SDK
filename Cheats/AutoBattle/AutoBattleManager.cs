...
namespace ASWDEBUG.Cheats.AutoBattle
{
    ...
    public static class AutoBattleManager
    {
        ...
        private static bool _autoUseOverrideActive;
        private static bool _autoUsePreviousEnabled;
        ...
        {
            Settings.AutoBattleEnabled = !Settings.AutoBattleEnabled;
            FileLogger.Log("AUTO-BATTLE", "enabled=" + Settings.AutoBattleEnabled);
            if (!Settings.AutoBattleEnabled)
            {
                ResetRuntime("已关闭");
            }
        }
        ...
        {
            Settings.AutoBattleLinkAutoUse = !Settings.AutoBattleLinkAutoUse;
            if (!Settings.AutoBattleLinkAutoUse) RestoreAutoUseIfNeeded();
        }

        public static void ToggleDebugLog()
        {
            Settings.AutoBattleDebugLog = !Settings.AutoBattleDebugLog;
        }

        public static void SetStrategy(int index)
        {
            Settings.AutoBattleStrategyMode = Mathf.Clamp(index, 0, StrategyNames.Length - 1);
        }

        public static void SetAccuracy(int index)
        {
            Settings.AutoBattleAccuracyMode = Mathf.Clamp(index, 0, AccuracyNames.Length - 1);
        }
        ...
        {
            if (!Settings.AutoBattleLinkAutoUse) return;
            try
            {
                AutoUseManager.EnsureLoaded();
                if (!_autoUseOverrideActive)
                {
                    _autoUsePreviousEnabled = AutoUseManager.Enabled;
                    _autoUseOverrideActive = true;
                }
                if (!AutoUseManager.Enabled)
                {
                    AutoUseManager.Enabled = true;
                    FileLogger.Log("AUTO-BATTLE", "AutoUse linked: enabled existing rule engine");
                }
            }
            catch (Exception ex)
            {
                FileLogger.Log("AUTO-BATTLE", "AutoUse link failed: " + ex.Message);
            }
        }

        private static void RestoreAutoUseIfNeeded()
        {
            if (!_autoUseOverrideActive) return;
            try
            {
                AutoUseManager.Enabled = _autoUsePreviousEnabled;
            }
            catch
            {
            }
            _autoUseOverrideActive = false;
        }
        ...
        private static float ScoreTarget(Character player, Character ch, TargetSense sense)
        {
            ...
            if (Settings.AutoBattleStrategyMode == 1) score -= Mathf.Min(5f, sense.Distance * 0.2f);
            if (Settings.AutoBattleStrategyMode == 2 && sense.Distance < 6f) score += 3f;
            return score;
        }
        ...
        private static void SwitchTarget(Character next, TargetSense sense, string reason)
        {
            ...
            if (Settings.AutoBattleDebugLog)
            {
        ...
        private static TargetSense BuildSense(Character player, Character target, Camera cam)
        {
            ...
            if (!sense.FireLineOfSight && sense.LineOfSight && sense.Distance <= 14.0f && !sense.HighGroundBlocked)
            {
                sense.FireLineOfSight = true;
                sense.FirePoint = sense.AimPoint;
            }
        ...
        private static Vector3 GetAimPoint(Character target, Camera cam, Vector3 basePoint, bool allowLower)
        {
            ...
            {
                int accuracy = Mathf.Clamp(Settings.AutoBattleAccuracyMode, 0, AccuracyNames.Length - 1);
        ...
        private static bool ShouldKite(Character player, TargetSense sense)
        {
            ...
            if (Settings.AutoBattleStrategyMode == 2 && sense.Distance < 8.0f) return true;
            if (Settings.AutoBattleStrategyMode == 0 && sense.Distance < 3.8f) return true;
            try
            {
        ...
        private static Vector3 SelectSearchPoint(Character player, Character target, TargetSense sense)
        {
            ...
            float seekDistance = Settings.AutoBattleStrategyMode == 1 ? 4.0f : 5.8f;
        ...
        private static Vector3 SelectEngagePoint(Character player, Character target, TargetSense sense, bool shieldFront, bool kite)
        {
            ...
            else
            {
                ...
                fromTarget.Normalize();
                float engageDistance = Settings.AutoBattleStrategyMode == 1 ? 3.1f : 4.8f;
                Vector3 side = Vector3.Cross(Vector3.up, fromTarget);
                if (side.sqrMagnitude < 0.01f) side = player.transform.right;
                side.y = 0f;
                side.Normalize();
        ...
        private static bool AimAt(Character player, Character target, Camera cam, TargetSense sense)
        {
            try
            {
                ...
                float tolerance = Settings.AutoBattleAccuracyMode == 0 ? 4.2f : (Settings.AutoBattleAccuracyMode == 1 ? 6.2f : 8.4f);
        ...
        {
            if (!Settings.AutoBattleDebugLog) return;
        ...
        private static bool ShouldFire(Character player, Character target, TargetSense sense, bool aimReady)
        {
            ...
            float fallbackPixels = Settings.AutoBattleAccuracyMode == 0 ? 96f :
                (Settings.AutoBattleAccuracyMode == 1 ? 122f : 150f);
            ...
            float skipChance = Settings.AutoBattleAccuracyMode == 0 ? 0.030f : (Settings.AutoBattleAccuracyMode == 1 ? 0.065f : 0.115f);
        ...
        {
            if (Settings.AutoBattleStrategyMode == 1) return UnityEngine.Random.Range(0.08f, 0.18f);
            if (Settings.AutoBattleStrategyMode == 2) return UnityEngine.Random.Range(0.18f, 0.34f);
        ...
        private static float NextFireDelayForWeapon(WeaponType type)
        {
            ...
            {
                if (Settings.AutoBattleStrategyMode == 1) return UnityEngine.Random.Range(0.045f, 0.090f);
                if (Settings.AutoBattleStrategyMode == 2) return UnityEngine.Random.Range(0.080f, 0.155f);
        ...
        {
            float interval = Settings.AutoBattleDebugLog ? LogInterval : 2.5f;
            ...
            try
            {
                ...
                string line =
                   ...
                   " path=" + LastPath +
                   " action=" + action;
                if (Settings.AutoBattleDebugLog) line += " pathDetail=" + LastPathDetail;
        ...
        private static void SetPathResult(string path, string detail, bool important)
        {
            ...
            if (!important && !Settings.AutoBattleDebugLog) return;
            if (Time.time < _nextPathDiagLogTime) return;
            _nextPathDiagLogTime = Time.time + (Settings.AutoBattleDebugLog ? 0.35f : 1.25f);
