...
namespace ASWDEBUG.Cheats.SurvivalBot
{
    ...
    internal static class LocalNavigationCombatTest
    {
        ...
        private static ObjectBaseInfo CreateSniperInfo()
        {
            ...
            info.sub_type = 2;
            info.range = 180f;
            info.cool_down = 0.08f;
            info.fire_time = 0.02f;
            info.ammo_one_clip = 20;
            info.ammo_count = 200;
            info.ammo_in_clip = 20;
            info.ammo_count_current = 200;
            info.is_sight = true;
            info.sight = 1;
            info.zoom_time = 0.08f;
            info.zoom_value = 18f;
            info.sight_info = new List<SightInfo> { new SightInfo(18f, 1f, 0f) };
            info.cool_down_origial = info.cool_down;
            info.range_origial = info.range;
            info.fire_time_origial = info.fire_time;
            info.cooling = -1f;
