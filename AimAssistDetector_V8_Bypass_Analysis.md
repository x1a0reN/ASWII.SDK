...
2026-07-22 的最新实现进一步限制为“持续低偏差段”处理：连续少于 3 个低值时完全
保留原生数据；持续段只调整较早的样本，最后一个样本保持原值，并用带小幅修正的平滑
收敛轨迹连接到开火瞬间精度。同一帧内相同目标和命中目标复用同一组调整值，避免霰弹
等多包射击在一次捕获中生成互相矛盾的历史。处理前还会验证精度编码校验位；不合法、
负值 sentinel、原生 missing、数组长度、目标、开火瞬间精度和命中字段均不改写。
