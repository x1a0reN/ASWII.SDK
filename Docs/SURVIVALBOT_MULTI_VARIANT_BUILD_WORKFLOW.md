...
6. 不创建备份；目标哈希不同时直接覆盖自用版，哈希相同时跳过写入，并逐项核验。
...
| 突击兵每局开局一次隐身 | 保留 | 保留 |
| 固定 5 秒人数采集（无面板配置） | 保留 | 保留 |
| 突击兵 40m 隐身预判 | 保留 | 保留 |
| 近距离强锁立即开火、稳定开镜及主武器择优 | 保留 | 保留 |
| 可见威胁强制接管及突击兵受伤隐身兜底 | 保留 | 保留 |
| 攻击期间安全持续移动与强锁 | 保留 | 保留 |
| `.aswnav` 边界索引与分帧精确悬崖搜索 | 保留 | 保留 |
| RAIN 精确路线收尾分帧（0.10m + 实时物理校验） | 保留 | 保留 |
| RAIN 局部绕障搜索分帧（校验条件不降级） | 保留 | 保留 |
| 猎杀目标平滑预瞄与强锁前收敛门槛 | 保留 | 保留 |
| 躲避路径敌方视线审计及致死悬崖验证 | 保留 | 保留 |
| `FightState` 无视挂机（默认开启，可在面板关闭） | 保留 | 保留 |
...
- 任一版本缺少 RAIN 路线分帧收尾、密集物理探针或完整验证阶段；
- DLL 内的版本标识与目标版本不一致；
- 构建、打包或部署后的任一文件哈希不一致。
